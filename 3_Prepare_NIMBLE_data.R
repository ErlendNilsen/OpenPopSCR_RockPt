...
################################################################################
## 1: Filtering the relevant data. In this case, 
## only data from the spring sampling
################################################################################
...
################################################################################
## 2. Create the y-array, holding the observation data
...
#### Expanding the tibble to include also detectors where there are no 
#### observations - also those that are not sampled; 

cell_numerical <- tibble(cell_numerical=seq(1,400))

temp_rpt2 <- full_join(cell_numerical, temp_rpt) %>%
  filter(!is.na(cell_numerical)) %>%
  mutate(verbatimEventDate=factor(verbatimEventDate, 
                                  levels=c("2014_spring",
...
ind.augmented <- 1.2
y.augmented <- array(0, dim=c((dim(y.alive)[1]*ind.augmented), 
                        dim(y.alive)[2], dim(y.alive)[3]))
...
################################################################################
### 3. Creating lowerHabCoords & upperHabcoords
################################################################################

## The bounding box around the study area
bounding_box <- tibble(x=c(408000, 413000), y=c(7148000, 7153000)) 

lowerHabCoords <- tibble(x=rep(seq(min(bounding_box$x), max(bounding_box$x)-250, 
                                   length.out = 20), 20), 
                         y=rep(seq(max(bounding_box$y)-250, min(bounding_box$y), 
                                   length.out = 20), each=20))


upperHabCoords <- tibble(x=rep(seq(min(bounding_box$x)+250, max(bounding_box$x), 
                                   length.out = 20), 20), 
                         y=rep(seq(max(bounding_box$y), min(bounding_box$y)+250, 
                                   length.out = 20), each=20))

################################################################################
#### 4. Creating detCovs; we use track logs for each detector each session
################################################################################
...
################################################################################
#### 5 Creating the detector.xy
################################################################################

temp_xy <- event_table_use %>% filter(eventRemarks=="Primary_session" & 
                                        verbatimEventDate=="2014_spring") 

temp_xy_sp <- SpatialPoints(coords=cbind(temp_xy$decimalLongitude,
                                         temp_xy$decimalLatitude), 
...
################################################################################
### 5. Creating habitatIDDet
################################################################################

habitatIDDet <- matrix(seq(1,400), ncol=20, byrow=T)

################################################################################
### 5 Creating sparseY array and nbdetections
################################################################################
